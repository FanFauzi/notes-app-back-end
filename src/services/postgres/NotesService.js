...
class NotesService {
  ...
  async getNotes(owner) {
    try {
      // mendapatka catatan dari cache
      const result = await this.cacheService.get(`notes:${owner}`);
      return JSON.parse(result);
    } catch (error) {
      // bila gagal, teruskan dengan mendapatkan catatan dari database
      const query = {
  ...
  async editNoteById(id, { title, body, tags }) {
    ...
    const query = {
      ...
      values: [title, body, tags, updatadAt, id],
    };

    const result = await this._pool.query(query);

    if (!result.rows.length) {
      throw new NotFoundError('Gagal memperbarui catatatn. Id tidak ditemukan');
    }
